  ...
  async run({ ticks, time }) {
    ...
    try {
      if (openTrades.length < this.maxActiveTrades) {
        if (
          avgSrt > avgLng &&
          ((previousBelow && oscilation < -100 && oscilation > -200) || (signal.shouldBuy() && adx > 25 && adx < 30))
        ) {
          this.onBuySignal({ price, time })
        }
      } else if (
        avgSrt < avgLng &&
        ((!previousBelow && oscilation > 100 && oscilation < 200) || (signal.shouldSell() && adx < 20))
      ) {
        openTrades.forEach((p) => {
          if (price - price * fee > p.enter.price * fee) {
